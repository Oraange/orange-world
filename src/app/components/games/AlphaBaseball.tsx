    ...
    return (
        <div className="max-w-6xl mx-auto">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8">
                <h2 className="text-2xl font-semibold text-blue-900 mb-2">단어 야구 게임</h2>
                <p className="text-blue-800">
                    5글자 영어 단어를 맞춰보세요! 글자가 맞으면 스트라이크, 글자는 틀렸지만 단어에 포함되어 있으면 볼, 단어에 포함되지 않으면 아웃입니다. 6번의 기회 안에 단어를 맞히면 승리합니다!
                </p>
            </div>
            {/* 게임 인터페이스 */}
            <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                    {history.map((result, index) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-gray-50 border border-gray-200 rounded-lg">
                            <span className="text-2xl font-bold text-gray-900">{result.guess}</span>
                ...
                {gameStatus !== "playing" && (
                    <div className="mt-6 p-4 bg-green-100 border border-green-200 rounded-lg text-center">
                        {gameStatus === "won" ? (
                            <h3 className="text-2xl font-bold text-green-800">🎉 축하합니다! 단어를 맞히셨습니다! 🎉</h3>
                        ) : (
                            <h3 className="text-2xl font-bold text-red-800">😞 아쉽네요! 정답은 "{answer.toUpperCase()}"였습니다. 😞</h3>
                        )}
                        <button
                            onClick={initGame}
